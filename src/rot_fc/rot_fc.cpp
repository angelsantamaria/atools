...
void v2aaxis(const Vector3d& v, double& angle, Vector3d& axis, MatrixXd& Aangle_v, MatrixXd& Aaxis_v)
{
	...
	if (angle>EPS)
	{
		Aaxis_v.row(0) << (1/angle)-(axis(0,0)*axis(0,0))/angle, -axis(0,0)/angle*axis(1,0), -axis(0,0)/angle*axis(2,0);
		Aaxis_v.row(1) << -axis(0,0)/angle*axis(1,0), (1/angle)-(axis(1,0)*axis(1,0))/angle, -axis(1,0)/angle*axis(2,0);
		Aaxis_v.row(2) << -axis(0,0)/angle*axis(2,0), -axis(1,0)/angle*axis(2,0), (1/angle)-(axis(2,0)*axis(2,0))/angle;
	}	
}
...
void v2R(const Vector3d& v, Matrix3d& R)
{
	...
	R = ca*Matrix3d::Identity() + M_sk + ((1-ca)*axis)*axis.transpose();
}
...
void v2q(const Vector3d& v, Quaterniond& q, MatrixXd& Q_v)
{
	...
	if (a<EPS)
	{
		q.w() = 1-pow(v.norm(),2/8);
		q.vec() = v/2;
		Q_v.row(0) << -1/4*v.transpose();
...
void aaxis2q(const double& angle, const Vector3d& axis, Quaterniond& q, MatrixXd& Q_angle)
{
	...
	Q_angle << -sin(angle/2)/2, (cos(angle/2)/2)*axis;
}
void aaxis2q(const double& angle, const Vector3d& axis, Quaterniond& q, MatrixXd& Q_angle, MatrixXd& Q_axis)
{
	...
	Q_axis << 0,0,0,sin(angle/2)*Matrix3d::Identity();
}

void w2omega(const Vector3d& w, Matrix4d& Omega)
{
	Omega.row(0) << 0,-w.transpose();
	Omega.col(0) << 0,w;
	Matrix3d M_sk(3,3);
	v2skew(-w,M_sk);
...
void q2aaxis(const Quaterniond& q, double& angle, Vector3d& axis, MatrixXd& Aangle_q)
{
	...
	Aangle_q << -2/sqrt(1-q.w()*q.w()),0,0,0;	
}
